...
def current_clamp_sweep_qc_features(sweep, is_ramp):
    ...
    if autobias_v and not is_ramp:
        pre_vm_delta_mv = qcf.measure_vm_delta(autobias_v, mean_first_stability_epoch)
        post_vm_delta_mv = qcf.measure_vm_delta(autobias_v, mean_last_stability_epoch)
        qc_features["vm_autobias_delta_mv"] = max(pre_vm_delta_mv, post_vm_delta_mv)
    else:
        qc_features["vm_autobias_delta_mv"] = None

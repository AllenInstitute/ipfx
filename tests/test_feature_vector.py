...
def test_step_subthreshold_interpolation():
    ...
    output = fv.step_subthreshold(
        amp_sweep_dict, target_amps, start=2, end=4, extend_duration=1, 
...
def test_subthresh_norm_normalization():
    ...
    output = fv.subthresh_norm(
        amp_sweep_dict, deflect_dict, start=t[0], end=t[-1], target_amp=-10, 
...
def test_subthresh_depol_norm_normalization():
    ...
    output = fv.subthresh_depol_norm(
        amp_sweep_dict, deflect_dict, start=t[0], end=t[-1],
...
def test_first_ap_correct_section():
    ...
    window_length = 5
    ap_v, ap_dv = fv.first_ap_vectors(
        [test_sweep], [test_info], target_sampling_rate=sampling_rate, 
        window_length=window_length
    )

    assert np.array_equal(
        ap_v, 
        test_sweep.v[test_spike_index:test_spike_index + window_length]
    )
...
def test_first_ap_resampling():
    ...
    window_length = 10
    ap_v, ap_dv = fv.first_ap_vectors(
        [test_sweep], [test_info], target_sampling_rate=sampling_rate / 2, 
        window_length=window_length
    )
...
def test_inst_freq_between_sweep_interpolation():
    ...
    si_list = [None, available_list[0], None, available_list[1], None]
    output = fv.inst_freq_vector(
        si_list, start=start, end=end, width=width
    )
...
def test_spike_feature_between_sweep_interpolation():
    ...
    assert np.all(
        output[2 * n_bins:3 * n_bins] == np.mean(test_feature_values)
    )
...
def test_identify_sub_depol_levels_without_subthreshold_sweeps():
    ...
    depolarizing_spiking = [
        a for a, r in zip(test_input_amplitudes, test_avg_rate)
        if a > 0 and r > 0
    ]
...
def test_isi_shape_skip_short():
    ...
    assert isi_norm[0] == (
        test_sweep.v[
            test_fast_trough_index[1]: 
            test_fast_trough_index[1] + test_subsample
        ].mean() - test_threshold_v[1]
    )
